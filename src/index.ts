...
app.use((req, _, next) => {
    console.log(`\nRequest event received for url "${req.url}" with method "${req.method.toUpperCase()}" at time "${new Date}"\n`);
...
app.post('/update-user/:user_id', async (req, res) => {
    const updated = updateUser((await getUserByID(req.params.user_id))!, req.body);
